...
const int64 nChainStartTime = 1395965503; 
...
const string strMessageMagic = "SuperHashToken Signed Message:\n";
...
{
	int64 nSubsidy = 50000000000 * COIN;
    int64 nMinSubsidy = 100000 * COIN;
	
{
  // Subsidy is cut in half every 500 blocks
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    // SuperHashToken: reward for coin-year is cut in half every 64x multiply of PoS difficulty
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal SuperHashToken miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in SuperHashToken
    bool fStrictPayToScriptHash = true; // Always active in SuperHashToken
    ...
    // ppcoin: fees are not collected by miners as in SuperHashToken
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SuperHashToken", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
// Genesis block
        const char* pszTimestamp = "SuperHashToken Born on 3-27-2014.";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:SuperHashToken genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("de0c7b7ee46f57397182a5235a7fe9ff770e02fbfdef41dd02c833ad614a8930"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("SuperHashTokenMiner : proof-of-work not meeting target");

    //// debug print
    printf("SuperHashTokenMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SuperHashTokenMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("SuperHashTokenMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadSuperHashTokenMiner(void* parg);

static bool fGenerateSuperHashTokens = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void SuperHashTokenMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("SuperHashToken-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateSuperHashTokens || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateSuperHashTokens) && !fProofOfStake)
                return;
        }
        ...
        printf("Running SuperHashTokenMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadSuperHashTokenMiner(void* parg)
{
    ...
    try
    {
        ...
        SuperHashTokenMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadSuperHashTokenMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadSuperHashTokenMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadSuperHashTokenMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateSuperHashTokens(bool fGenerate, CWallet* pwallet)
{
    fGenerateSuperHashTokens = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateSuperHashTokens = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d SuperHashTokenMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadSuperHashTokenMiner, pwallet))
                printf("Error: NewThread(ThreadSuperHashTokenMiner) failed\n");
            Sleep(10);
        }
    }
}
